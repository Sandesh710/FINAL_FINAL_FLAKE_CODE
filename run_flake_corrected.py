...
def parse_flake_nml(nml_path):
    ...
    # Temperature initialization will be done after reading lake parameters
    # (see after LAKE_PARAMS section below)
    ...
    # Compute initial temperature profile based on FLAKE model theory
    # Following FORTRAN initialization: establish stratification from shape factor C_T
    # T_mnw is the depth-averaged temperature computed from the profile integral
    C_T_0 = C_T_min  # 0.5 (from parameters)
    
    # For nearly uniform initial conditions, FORTRAN applies slight stratification
    if abs(T_wML_0 - T_bot_0) < 0.01:  # Uniform initial T from NML file
        # Apply stratification: T_bot slightly cooler than T_wML
        # Based on test file at t=0: T_wML=4.000, T_bot=3.980, T_mnw=3.995
        delta_T = 0.02  # Temperature drop to bottom (in Kelvin)
        T_bot_0 = T_wML_0 - delta_T
        
        # Compute T_mnw from profile integral
        h_thermo = depth_w - h_ML_in  # Thermocline thickness
        if h_thermo > 0:
            # With shape factor C_T, thermocline average temperature is:
            # T_thermo_avg = T_wML - 0.5 * C_T * (T_wML - T_bot)
            T_thermo_avg = T_wML_0 - 0.5 * C_T_0 * (T_wML_0 - T_bot_0)
            # Depth-weighted average
            T_mnw_0 = (T_wML_0 * h_ML_in + T_thermo_avg * h_thermo) / depth_w
        else:
            T_mnw_0 = T_wML_0
    else:
        # Already stratified, compute T_mnw from profile
        h_thermo = depth_w - h_ML_in
        if h_thermo > 0:
            T_thermo_avg = T_wML_0 - 0.5 * C_T_0 * (T_wML_0 - T_bot_0)
            T_mnw_0 = (T_wML_0 * h_ML_in + T_thermo_avg * h_thermo) / depth_w
        else:
    ...
    print(f"✅ Initial temperature profile (corrected):")
    print(f"   T_wML = {T_wML_0:.6f} K ({T_wML_0-273.15:.6f} °C)")
    print(f"   T_mnw = {T_mnw_0:.6f} K ({T_mnw_0-273.15:.6f} °C)")
    print(f"   T_bot = {T_bot_0:.6f} K ({T_bot_0-273.15:.6f} °C)")
    print(f"   Stratification: ΔT = {(T_wML_0-T_bot_0):.6f} K")
